    ...
    useEffect(() => {
        ...
        try {
            ...
            const hasSuperAdmin = roles.some(
                role => typeof role === "string" && role.toLowerCase() === "superadmin"
            );
        ...
        <>
            ...
            >
                ...
                <nav className="mt-6 flex flex-col flex-grow">
                    ...
                    <NavLink to="/Category" className={navClass} onClick={() => setOpen(false)}>
                        <BiCategory />
                        {open && <span>Categories</span>}
                    </NavLink>
                    <NavLink to="/SubCategory" className={navClass} onClick={() => setOpen(false)}>
                        <MdCategory />
                        {open && <span>SubCategories</span>}
                    </NavLink>
                    <NavLink to="/Specialization" className={navClass} onClick={() => setOpen(false)}>
                        <GiSkills />
                    ...
                    <NavLink
                        to={isSuperAdmin ? "/Log" : "#"}
                        className={isSuperAdmin ? navClass : "flex text-gray-500 items-center gap-4 p-4 cursor-not-allowed"}
                        onClick={() => {
                            if (isSuperAdmin) setOpen(false);
                        }}
                    >
                        <ClipboardList size={18} />
                        {open && <span>Logs</span>}
                    </NavLink>
                    <NavLink to="/Report" className={navClass} onClick={() => setOpen(false)}>
                        <ImWarning size={18} />
